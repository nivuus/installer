...
**Also flagged, not acted on**: `/media/data` and `/media/backup` are mounted `data=writeback,barrier=0` (see `/etc/fstab`) — with repeated instant power cuts that is a real data-loss exposure, and the swapfile lives on one of them. And the 9 `winvm-proxy-*.socket` entries in `/etc/systemd/system/sockets.target.wants/` are regular files, not symlinks, so systemd ignores them (`is not a symlink, ignoring`).
